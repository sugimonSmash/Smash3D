...
bool BufferObject::BufferSubData(GLintptr offset, GLsizeiptr size, const GLvoid* data)
{
	if (offset + size >= this->size) {
		std::cerr << "[警告]" << __func__ << ": 転送領域がバッファサイズを超えています.\n";
...
*
* 描画、VertexAttribPointer()、ResetVertexAttribPointer()を呼び出す前に、この関数を実行してVAOを割り当てる.?
*
...
*
* 描画、VertexAttribPointer()、ResetBertexAttribPointer()が終わったら、この関数を実行してバインドを解除する?
*
